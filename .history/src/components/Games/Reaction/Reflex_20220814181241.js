...
export default function Reflex() {
    ...
    function addData(){
        ...
        games_parsed.reflex.average = ((games_parsed.reflex.times.reduce((a,b)=>a+b,0)+time)/games_parsed.reflex.times.length)
        console.log("average"+(games_parsed.reflex.times.reduce((a,b)=>a+b,0)+time)/games_parsed.reflex.times.length)
        localStorage.setItem('games', JSON.stringify(games_parsed))
