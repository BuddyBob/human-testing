...
export default function Memory(){
  ...
  async function dim(i){
    document.getElementsByClassName("cell"+problemSet[i].value)[0].classList.add('dim')
    document.getElementsByClassName("cell"+problemSet[i].value)[0].classList.remove('light')
  }
  ...
  async function lightUp(){
    ...
    for (let i = 0; i < problemSet.length; i++){
      document.getElementsByClassName("cell"+problemSet[i].value)[0].classList.remove('dim')
      document.getElementsByClassName("cell"+problemSet[i].value)[0].classList.add('light')
  ...
  if (problem){
    ...
    for (let i = 0; i < newProblemSet.length; i++){
      ...
      setProblemSet(problemSet.push({
        id: problemSet.length,
        value: newProblemSet[i]
      }));
